...
async function Zone_Code(zoneName) {
  try {
    const query =
      "SELECT Zone_Code FROM parent_zone_mapping WHERE Zone_Name = ?";
...
const getStorage = (maxID) => {
  const uploadFolderLowRes = path.join(
    "\\\\192.168.90.32\\Images\\",
    `${maxID}\\LowRes`
  );
...
const copyFilesToHighRes = async (maxID) => {
  ...
  const uploadFolderHighRes = path.join(
    "\\\\192.168.90.32\\Images\\",
    `${maxID}\\HighRes`
  );
    ...
    upload(req, res, async (err) => {
      if (err) {
        if (err instanceof multer.MulterError) {
          ...
          return res
            .status(500)
            .json({
              error: "Multer error occurred while uploading files to LowRes",
            });
        }
        ...
        return res
          .status(500)
          .json({
            error: "An unknown error occurred while uploading files to LowRes",
          });
      }
      ...
      try {
        ...
        return res
          .status(200)
          .json({
            message: "Files uploaded successfully to both LowRes and HighRes",
            files: req.files,
          });
      } catch (copyError) {
        ...
        return res
          .status(500)
          .json({ error: "Error copying files to HighRes" });
      }
    });
  } catch (error) {
...
exports.getProducts = async (req, res) => {
  try {
    const [rows, fields] = await pool.query(
      'SELECT * FROM mas_product WHERE status = "A" '
    );
    res.json(rows);
  } catch (error) {
...
exports.getZone = async (req, res) => {
  try {
    const [rows, fields] = await pool.query(
      ' SELECT m.Zone_Code, m.Zone_Name FROM mas_zone m where Zone_Code<>"ALL" '
    );
    res.json(rows);
  } catch (error) {
    console.error("Error fetching products:", error);
...
exports.assignUser = async (req, res) => {
  try {
    ...
    WHERE z.Zone_Name = ? And u.Status='A' And u.DEPAT_NAME='News Desk' `;
...
exports.insertArticle = async (req, res) => {
  ...
  try {
    ...
    const ImageCaptionvalue = await ImageCaption(
      filenames,
      finalCaption,
      parentid
    );
    ...
    const query = `
      INSERT INTO news_details_new (
        ...
        '${Created_user}', '${pagename}', '${Storyname}', '${encodeToBase64(
      HeadKicker
    )}', '${encodeToBase64(Head)}','${encodeToBase64(
      HeadDesk
    )}', '${encodeToBase64(Byline)}', '${encodeToBase64(
      Dateline
    )}', '${encodeToBase64(paragraph)}',
        'RE', 'TheHindu', '${xml_parent_action}', 'N', 'N', 'Y', 'Y', '${Status}',
        DATE_ADD(CURDATE(), INTERVAL 1 DAY),DATE_ADD(CURDATE(), INTERVAL 1 DAY),
        '${filenames}', '${path}', '${encodeToBase64(
      finalCaption
    )}', '${filenames}', '${TotalXml}',
...
exports.updateArticle = async (req, res) => {
  ...
  try {
    ...
    const finalCaptionString = Array.isArray(finalCaption)
      ? finalCaption.join(" ")
      : finalCaption;
    //  console.log("finalCaptionString",finalCaptionString);

    const updateCaption =
      finalCaptionString && finalCaptionString.trim() !== ""
        ? finalCaptionString
        : caption;
...
exports.getImageName = async (req, res) => {
  try {
    ...
    const input_folder = path.join(
      `\\\\192.168.90.32\\Images\\${folderName}\\LowRes`
    );
...
exports.fetchnews = async (req, res) => {
  ...
  try {
    ...
    const query = `SELECT * FROM news_details_new WHERE IssueDate = ? AND Product = ? AND Zone_Code LIKE ? AND desk_type = ?`;
...
exports.adAuth = async (req, res) => {
  try {
    ...
    if (formData !== null) {
      const redirectURL = `http://192.168.90.139:3000/user?formData=${encodeURIComponent(
        formDataJSON
      )}`;
...
exports.getImages = async (req, res) => {
  ...
  if (!date || !zone || !product || !page || !edition) {
    return res
      .status(400)
      .json({ error: "Date, Zone, Product, Page, and Edition are required" });
  }

  // Construct the file path
  const filePath = path.join(
    "\\\\192.168.90.32\\EditorialImage\\Image_Preview",
    date,
    zone,
    `${product}_${date}_${zone}_${edition}_${page}.jpg`
  );
...
exports.getPdf = async (req, res) => {
  ...
  const filePath = path.join(
    ...
    date,
    zone,
    `${product}_${date}_${zone}_${edition}_${page}.pdf`
  );
...
exports.getPlanData = async (req, res) => {
  ...
  if (!issueDate || !zoneId || !productId) {
    return res
      .status(400)
      .json({ error: "Date, Zone ID, and Product ID are required" });
  }
...
async function getPlanId(issueDate, productId, zoneId) {
  try {
    ...
    const query =
      "SELECT a.plan_id FROM plan_hdr a WHERE a.issue_dt = ? AND a.Product_ID = ? AND a.ZONE_ID = ?";
...
exports.updatePageCompose = async (req, res) => {
  const { issueDate, zoneId, productId, pageName, newStatus, newNoOfRevokes } =
    req.body;
  console.log(
    "Update value:",
    issueDate,
  ...
  try {
    if (
      ...
      return res
        ...
        .json({
          error:
            "Date, Product ID, Zone ID, Page Name, Status, and No of Revokes are required",
        });
    }
    ...
    const query1 =
      "UPDATE plan_page SET STATUS = ?  WHERE Plan_ID = ? AND Page_id = ?";
  ...
  try {
    ...
    if (!issueDate || !productId || !zoneId || !Array.isArray(activeUpdates)) {
      return res
        ...
        .json({
          error:
            "Issue Date, Product ID, Zone ID, and Active Updates are required",
        });
    }
    ...
    const updatePromises = activeUpdates.map(async ({ pageId, isActive }) => {
      ...
      const query =
        "UPDATE plan_page SET ACTIVE = ? WHERE Plan_ID = ? AND Page_id = ?";
    ...
    const totalUpdatedRows = results.reduce(
      (acc, { rowsUpdated }) => acc + rowsUpdated,
      0
    );
